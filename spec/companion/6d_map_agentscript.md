...
| `protocol_id` | **PROVISIONAL.** No standards-assigned code point exists: NPAMP-REG §6 assigns only `0x01`–`0x04`, and AgentScript is listed OPAQUE-READY / mapping-planned in the companion index, not assigned. Until a code point is assigned under NPAMP-REG §8 (First Come First Served, range `0x05`–`0x0F`), this mapping uses the **experimental** value `0x3E` (range `0x10`–`0x7F`; NPAMP-REG §7.1), which carries **no cross-domain meaning** and MUST be agreed out-of-band (§8). |
